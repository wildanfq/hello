...
## 6. Ringkasan Alur Sistem

Secara keseluruhan, proses kerja sistem dapat diringkas sebagai berikut:

```text
Power On
    │
    ▼
QEMU memuat hello.bin ke RAM
(0x80000000)
    │
    ▼
CPU mulai eksekusi di _start
    │
    ▼
Alamat UART dimuat ke t0
    │
    ▼
Alamat string dimuat ke t1
    │
    ▼
Baca karakter dari memori
    │
    ▼
Karakter = 0x00 ?
 ├── Ya ──► Infinite Loop
...
## 7. Menghentikan Simulasi

Program ini tidak memiliki mekanisme keluar (*exit*) karena berjalan pada lingkungan *bare-metal*. Setelah seluruh karakter dicetak, CPU akan tetap berada dalam *infinite loop*.

Untuk menghentikan simulasi QEMU, tekan:
...
Kombinasi tersebut akan menutup emulator dan mengembalikan kontrol terminal ke sistem operasi host. Dengan demikian proses pengujian dapat dihentikan secara aman tanpa memaksa menutup terminal.
